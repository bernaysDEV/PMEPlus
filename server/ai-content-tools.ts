  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `أنت مختص في إنشاء محتوى وسائل التواصل الاجتماعي لبروبرتي ME.`,
        },
        {
          role: "user",
...
export async function analyzeTrends(
  ...
  try {
    ...
    const claudePromise = getAnthropicClient().messages.create({
      model: "claude-sonnet-4-5",
      max_tokens: 3000,
      messages: [
        {
...
⚠️ مهم: أرجع JSON فقط بدون أي شرح أو مقدمة.`);
    ...
    if (claudeResult.status === "fulfilled") {
      ...
      console.error(`❌ [Claude] Request failed: ${msg}`);
    }
