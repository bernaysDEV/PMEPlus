...
export async function generateTitle(content: string, language: "ar" | "en" = "ar"): Promise<string[]> {
  try {
    ...
    const SYSTEM_PROMPTS = {
      ar: `أنت محرر عناوين خبير في صحيفة "بروبرتي ME" السعودية. تتبع معايير صارمة لكتابة العناوين:
...
- لا تكرر نفس الكلمات في العناوين المختلفة`,
      en: `You are an expert headline editor at "Property ME" newspaper. Follow strict headline standards:
...
export async function chatWithAssistant(
  ...
  try {
    ...
    const systemPrompt = `أنت مساعد أخبار ذكي لبروبرتي ME. ساعد القراء في العثور على الأخبار والمعلومات. أجب بالعربية دائماً.
...
export async function generateSmartContent(newsContent: string, language: "ar" | "en" = "ar"): Promise<{
  ...
  try {
    ...
    const systemPrompts = {
      ar: `🎯 الدور: أنت محرر خبير في صحيفة "بروبرتي ME" السعودية، متخصص في كتابة الأخبار بأسلوب صحفي احترافي وسهل الفهم، يدعم تحسين محركات البحث (SEO) ويجذب القارئ العربي.
...
1. **العنوان الرئيسي (main_title):**  
   ...
   - عنوان قوي ومفصّل يشرح الخبر بوضوح.  
   - يبدأ بفعل قوي أو اسم فاعل.  
   - جذّاب، شامل، ومناسب لأسلوب صحيفة "بروبرتي ME".  
   - يتضمن كلمة مفتاحية رئيسية.  
...
- حافظ على الأسلوب الإخباري الرسمي لصحيفة "بروبرتي ME".  
...
أعد النتيجة بصيغة JSON فقط مع الحقول التالية:
{
  ...
  "suggested_category": ""
      ...
      en: `🎯 Role: You are an expert editor at "Property ME" newspaper, specializing in writing news articles with a professional journalistic style that is clear, SEO-optimized, and engaging for English-speaking readers.
...
1. **Main Title (main_title):**  
   ...
   - Strong, detailed headline that clearly explains the news.  
   - Start with a strong verb or active noun.  
   - Catchy, comprehensive, and suitable for "Property ME" newspaper style.  
   - Include a primary keyword.  
...
- Maintain the formal news style of "Property ME" newspaper.  
...
Return the result in JSON format only with the following fields:
{
  ...
  "suggested_category": ""
      ...
      en_translate: `🎯 Role: You are an expert bilingual editor and translator at "Property ME" newspaper, specializing in translating Arabic news to professional English journalism.
...
export async function rewriteAndEnhanceContent(
  ...
  try {
    const systemPrompts = {
      ar: `🎯 الدور: أنت محرر صحفي محترف في صحيفة "بروبرتي ME" السعودية، متخصص في إعادة صياغة وتحرير الأخبار بأسلوب احترافي يجذب القارئ العربي.
...
أعد النتيجة بصيغة JSON فقط:
{
  ...
  "improvements_summary": ["تحسين 1", "تحسين 2", ...]
      ...
      en: `🎯 Role: You are a professional editor at "Property ME" newspaper, specializing in rewriting and editing news with a professional style that engages English-speaking readers.
