...
// Unified TTS provider interface — supports Google TTS and ElevenLabs
type TTSProvider = {
  name: string;
...
function getActiveTTSProvider(): TTSProvider | null {
  // Selection: explicit TTS_PROVIDER env wins, otherwise prefer Google if configured
  ...
  if (provider !== 'elevenlabs') {
    ...
    if (google) {
      return {
        name: 'google',
        textToSpeech: (opts, timeoutMs) => google.textToSpeech(opts, timeoutMs),
        charLimit: 4500,
      };
    }
  }

  const eleven = getElevenLabsService();
  if (eleven) {
    return {
      name: 'elevenlabs',
      textToSpeech: (opts, timeoutMs) => eleven.textToSpeech(opts, timeoutMs),
      charLimit: 4000,
    };
  }
  return null;
...
const SCRIPT_TEMPLATES: Record<NewsletterTemplate, ScriptTemplate> = {
  [NewsletterTemplate.MORNING_BRIEF]: {
    introduction: (data) => `
      صباح الخير ومرحباً بكم في نشرة بروبرتي ME الصباحية ليوم ${data.date}.
    ...
    `,
    conclusion: (data) => `
      كانت هذه أبرز أخبار الصباح من بروبرتي ME.
      نتمنى لكم يوماً موفقاً، ونلقاكم في النشرة المسائية.
      للمزيد من الأخبار، تابعونا على تطبيق بروبرتي ME.
    `
  },
  
  [NewsletterTemplate.EVENING_DIGEST]: {
    introduction: (data) => `
      مساء الخير، أهلاً بكم في النشرة المسائية من بروبرتي ME.
  ...
  [NewsletterTemplate.WEEKLY_ANALYSIS]: {
    introduction: (data) => `
      أهلاً بكم في النشرة الأسبوعية التحليلية من بروبرتي ME.
      نقدم لكم تحليلاً معمقاً لأبرز أحداث الأسبوع الماضي وتأثيراتها المحتملة.
      معكم ${data.narrator || 'فريق بروبرتي ME الإخباري'}.
    `,
    articleSegment: (article, index) => `
    ...
    conclusion: (data) => `
      ...
      لا تنسوا متابعتنا على تطبيق بروبرتي ME للحصول على آخر الأخبار فور حدوثها.
    `
  },
  ...
  [NewsletterTemplate.BREAKING_NEWS]: {
    ...
    conclusion: (data) => `
      ...
      تابعوا تطبيق بروبرتي ME للحصول على آخر التحديثات.
    `
  },
  
  [NewsletterTemplate.TECH_UPDATE]: {
    introduction: (data) => `
      مرحباً بكم في نشرة بروبرتي ME التقنية.
    ...
    `,
    conclusion: (data) => `
      كانت هذه نشرة بروبرتي ME التقنية.
      للمزيد من أخبار التكنولوجيا، تابعونا على تطبيق بروبرتي ME.
    `
  },
  
  [NewsletterTemplate.BUSINESS_REPORT]: {
    introduction: (data) => `
      أهلاً بكم في التقرير الاقتصادي من بروبرتي ME.
  ...
  [NewsletterTemplate.SPORT_HIGHLIGHTS]: {
    introduction: (data) => `
      مرحباً بمحبي الرياضة، هذه نشرة بروبرتي ME الرياضية.
    ...
    conclusion: (data) => `
      ...
      تابعونا للمزيد من الأخبار الرياضية على تطبيق بروبرتي ME.
    `
  },
  
  [NewsletterTemplate.CUSTOM]: {
    introduction: (data) => data.customIntro || 'مرحباً بكم في نشرة بروبرتي ME.',
    articleSegment: (article, index) => `
      ${article.title}.
      ${article.aiSummary || article.excerpt || ''}
    `,
...
export class AudioNewsletterService extends EventEmitter {
  ...
  constructor() {
    ...
    this.ttsProvider = getActiveTTSProvider();
    this.activeJobs = new Map();
    if (this.ttsProvider) {
      console.log(`🔊 Audio newsletter using TTS provider: ${this.ttsProvider.name}`);
    }
  }
  ...
  private async processAudioGeneration(job: AudioGenerationJob): Promise<void> {
    try {
      // Refresh provider in case env was updated since boot
      if (!this.ttsProvider) {
        this.ttsProvider = getActiveTTSProvider();
      }
      if (!this.ttsProvider) {
        throw new Error('No TTS provider configured - please add GOOGLE_TTS_CREDENTIALS_JSON or ELEVENLABS_API_KEY');
      }
      ...
      // Chunk text for processing (provider-specific limit)
      const chunks = this.chunkText(script, this.ttsProvider.charLimit);
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        try {
          const audioBuffer = await this.ttsProvider.textToSpeech({
            text: chunk,
            voiceId,
            voiceSettings,
            model: modelId // Provider-specific (ignored by Google)
