...
export async function generateCalendarEventIdeas(
  ...
  try {
    ...
    const prompt = `أنت خبير تحريري في بروبرتي ME. لديك حدث "${eventTitle}" من نوع "${eventType}" في تاريخ ${arabicDate}.
...
أعد الإجابة بصيغة JSON فقط`;
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "أنت خبير تحريري ومتخصص في تحليل الأحداث وإنشاء محتوى إعلامي شامل باللغة العربية. أنت تعمل لبروبرتي ME الإخبارية."
        },
        {
          role: "user",
          content: prompt
...
أعد النتيجة بصيغة JSON فقط`;
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "أنت صحفي محترف تكتب مقالات إخبارية شاملة ومتوازنة باللغة العربية لبروبرتي ME."
        },
        {
          role: "user",
          content: prompt
