...
/**
 * Fire-and-forget helper that extracts geo locations from an article and persists
 * them to the `geoLocations` column. Use this from publish/create routes so the
 * footer's city links stay fresh without manual backfill runs.
 *
 * Skips work if `existingGeoLocations` already has entries.
 */
export function extractAndSaveGeoLocations(
  articleId: string,
  title: string,
  content: string | null | undefined,
  existingGeoLocations: unknown,
  context: string = 'article',
): void {
  if (Array.isArray(existingGeoLocations) && existingGeoLocations.length > 0) {
    return;
  }
  extractGeoLocations(title, content || '')
    .then(async (locations) => {
      await db
        .update(articles)
        .set({ geoLocations: locations })
        .where(eq(articles.id, articleId));
      console.log(
        `[GeoExtraction] Saved ${locations.length} locations for ${context} ${articleId}`,
      );
    })
    .catch((err) =>
      console.error(`[GeoExtraction] Error for ${context} ${articleId}:`, err),
    );
}
