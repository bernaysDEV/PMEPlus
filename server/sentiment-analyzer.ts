 ...
 */
function getPromptForLanguage(language: 'ar' | 'en', commentText: string): string {
  const prompts = {
...
function parseSentimentResponse(response: string): { sentiment: 'positive' | 'neutral' | 'negative'; confidence: number } {
  ...
  if (percentageMatch) {
    ...
    if (decimalMatch) {
      ...
      const confidenceMatch = response.match(/(?:confidence|ثقة):\s*(\d+(?:\.\d+)?)/i);
      if (confidenceMatch) {
 ...
 * @param language - Language of the comment ('ar' or 'en')
