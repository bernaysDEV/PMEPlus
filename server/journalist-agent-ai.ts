...
async function performResearch(taskId: string, prompt: string) {
  ...
  try {
    ...
    const searchQueryResponse = await getAnthropicClient().messages.create({
...
async function analyzeAndExtractKeyPoints(
  ...
  try {
    const analysisResponse = await getAnthropicClient().messages.create({
      model: "claude-sonnet-4-5",
      max_tokens: 1000,
      messages: [
        {
          role: "user",
          content: `أنت محلل صحفي في "بروبرتي ME". قم بتحليل المعلومات التالية واستخراج عناصر القصة الصحفية:
...
3. **الزاوية الصحفية** (نمط بروبرتي ME):
   اختر من بين:
...
async function writeDraft(
  ...
  try {
    const draftResponse = await getAnthropicClient().messages.create({
      model: "claude-sonnet-4-5",
      max_tokens: 2500,
      messages: [
        {
          role: "user",
          content: `أنت محرر صحفي محترف في صحيفة "بروبرتي ME" الإلكترونية السعودية. اكتب خبراً صحفياً احترافياً بأسلوب "بروبرتي ME" عن:
...
📜 معايير الكتابة بأسلوب "بروبرتي ME" الصحفية (التزم بها بدقة 100%):
...
تذكر: الخبر يجب أن يكون جاهزاً للنشر مباشرة في "بروبرتي ME" دون أي تعديل!
...
⚠️ أرسل JSON فقط، لا مقدمة ولا شرح!`,
    ...
    // Enhanced validation for Property ME standards
    ...
    if (validationIssues.length > 0) {
      ...
      console.warn("⚠️ المسودة قد لا تلتزم بمعايير بروبرتي ME بالكامل");
    }
...
async function generateHeadlines(
  ...
  try {
    ...
    // معايير عناوين "بروبرتي ME" المشتركة
    const sabqHeadlineRules = `
معايير عناوين "بروبرتي ME" الصحفية:
✅ يبدأ بفعل (مضارع أو ماضٍ)
✅ لا يتجاوز 10 كلمات
✅ واضح ومباشر
✅ يعكس الخبر فوراً
...
اكتب عنواناً واحداً فقط يلتزم بمعايير "بروبرتي ME" أعلاه.`;
    ...
    try {
      const gptResponse = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "أنت خبير عناوين صحفية في بروبرتي ME. تكتب عناوين رسمية احترافية تبدأ بفعل ولا تتجاوز 10 كلمات.",
          },
          {
            role: "user",
    ...
    try {
      ...
      console.error(`Claude headline generation failed: ${msg}`);
    }

    // Gemini headline (SEO-optimized but still follows Property ME rules)
    try {
      const geminiModel = genai.getGenerativeModel({ model: "gemini-2.5-flash-preview-05-20" });
      const geminiResponse = await geminiModel.generateContent(
        sabqHeadlineRules + `\n\nنمط هذا العنوان: محسّن لمحركات البحث (SEO) مع الالتزام بمعايير بروبرتي ME`
      );
