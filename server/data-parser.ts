...
/**
 * SECURITY NOTE — `xlsx` (SheetJS community build) has two open advisories
 * (Prototype Pollution + ReDoS) and ships no patched version on npm. The
 * decision recorded in `replit.md` ("xlsx posture") is to QUARANTINE rather
 * than replace: the only callers (`/api/data-stories/upload`) are gated by
 * authenticated editor/admin roles, multer caps file size at 10 MB, and the
 * helper below additionally caps row count and parse time. Anonymous /
 * unauthenticated callers can never reach this parser.
 */
const XLSX_MAX_ROWS = 50_000;
const XLSX_MAX_COLUMNS = 200;
const XLSX_MAX_PARSE_MS = 15_000;
...
export async function parseExcel(buffer: Buffer): Promise<ParsedDataset> {
  ...
  const checkBudget = () => {
    if (Date.now() - startedAt > XLSX_MAX_PARSE_MS) {
      throw new Error('تجاوزت معالجة ملف Excel الحد الزمني المسموح');
    }
  };
  try {
    // `cellDates: false` and `cellNF: false` keep the parser on its narrow
    // happy path; `bookVBA: false` refuses any embedded macros.
    ...
    if (rows.length > XLSX_MAX_ROWS) {
      throw new Error(`عدد الصفوف تجاوز الحد الأقصى المسموح (${XLSX_MAX_ROWS})`);
    }
    const firstRow = rows[0] && typeof rows[0] === 'object' ? rows[0] : {};
    if (Object.keys(firstRow).length > XLSX_MAX_COLUMNS) {
      throw new Error(`عدد الأعمدة تجاوز الحد الأقصى المسموح (${XLSX_MAX_COLUMNS})`);
    }
