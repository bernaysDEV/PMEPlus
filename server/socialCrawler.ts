...
function generateArticleHTML(article: any, baseUrl: string): string {
  ...
  const seoTitle = seoData.title || article.title || 'خبر من بروبرتي ميدل إيست';
  ...
  const structuredData = {
    ...
    "publisher": {
      ...
      "logo": {
        "@type": "ImageObject",
        "url": `${baseUrl}${BRAND_OG_IMAGE_PATH}`
      }
    }
  };
...
function generateOpinionArticleHTML(article: any, baseUrl: string): string {
  // Extract SEO data from article.seo JSON field or fallback to article fields
  const seoData = article.seo || {};
  const seoTitle = seoData.title || article.title || 'مقال رأي من بروبرتي ميدل إيست';
  // Support both camelCase and snake_case for aiSummary
...
function generateCategoryHTML(category: any, baseUrl: string): string {
  const seoTitle = `${category.nameAr} - بروبرتي ميدل إيست`;
  const seoDescription = category.description || `أخبار ${category.nameAr} على مدار الساعة من بروبرتي ميدل إيست`;
...
function generateReporterHTML(user: any, baseUrl: string): string {
  const fullName = user.firstName && user.lastName 
    ...
    : user.email;
  const seoTitle = `${fullName} - مراسل بروبرتي ميدل إيست`;
  const seoDescription = user.bio || `تابع أخبار ${fullName} على بروبرتي ميدل إيست`;
...
function generateIFoxHTML(baseUrl: string): string {
  const seoTitle = 'آي فوكس - بوابة الذكاء الاصطناعي | بروبرتي ميدل إيست';
...
function generateHomepageHTML(baseUrl: string): string {
  const seoTitle = 'Property Middle East - بروبرتي ميدل إيست - المنصة الأولى لأخبار العقار في الشرق الأوسط';
  const seoDescription = 'بروبرتي ميدل إيست هي المنصة الرائدة لأخبار العقارات في منطقة الشرق الأوسط. نحن نلتزم بأعلى المعايير لتقديم تغطية شاملة لسوق العقارات المتغير في المنطقة، مع تحليلات متخصصة وقصص فريدة تعزز فهمك لهذا القطاع المثير.';
  // Use the proper OG image (1200x630) for social sharing
  const seoImage = `${baseUrl}${BRAND_OG_IMAGE_PATH}`;
...
function generateMetaHTML(options: MetaHTMLOptions): string {
  const { 
    ...
    dir = 'rtl',
    locale = 'ar_SA',
    siteName = 'بروبرتي ميدل إيست - Property Middle East',
...
function generateEnglishArticleHTML(article: any, baseUrl: string): string {
  const seoData = article.seo || {};
  const seoTitle = seoData.title || article.title || 'News from Property Middle East';
...
export async function socialCrawlerMiddleware(
  ...
  const host = req.headers['host'] || BRAND_PRIMARY_DOMAIN;
  ...
  try {
    ...
    if (articleMatch) {
      ...
      await prepareSocialImage(rawImg).catch(() => {});
    ...
    if (opinionMatch) {
      ...
      await prepareSocialImage(rawOpImg).catch(() => {});
    ...
    if (enArticleMatch) {
      ...
      await prepareSocialImage(article.imageUrl || '').catch(() => {});
    ...
    // 4️⃣ Handle deep analysis (Omq) pages: /ai/omq/:id
    ...
    // 8️⃣ Handle iFox AI homepage: /ai/ifox
