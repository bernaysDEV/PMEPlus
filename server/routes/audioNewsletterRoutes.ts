...
// Resolve the active TTS provider (Google preferred, ElevenLabs fallback)
async function getActiveVoiceProvider() {
  ...
  if (explicit !== 'elevenlabs') {
    ...
    const g = getGoogleTTSService();
    if (g) return { name: 'google' as const, svc: g };
  }
  const { getElevenLabsService } = await import('../services/elevenlabs');
  const e = getElevenLabsService();
  if (e) return { name: 'elevenlabs' as const, svc: e };
  return null;
}
...
router.get('/voices', async (req, res) => {
  try {
    const provider = await getActiveVoiceProvider();
    if (!provider) {
      return res.status(503).json({ error: 'No TTS provider configured' });
    }
    const voices = await provider.svc.getVoices();
    res.json({ voices, provider: provider.name });
  } catch (error) {
...
// HARDENED: now also rate-limited via ttsLimiter (per-user/per-IP) so this
// expensive synthesis call can't be used as an amplifier against ElevenLabs.
router.post('/voices/test', ttsLimiter, async (req, res) => {
  try {
    ...
    const audioBuffer = await provider.svc.testVoice(
      voiceId,
      sampleText || 'مرحباً، هذا اختبار للصوت. سنقرأ لكم أهم الأخبار من بروبرتي ME اليوم.',
      settings
    );
...
<rss version="2.0" 
  ...
  <channel>
    ...
    <description>استمع إلى أهم الأخبار والتحليلات من بروبرتي ME</description>
    <link>${baseUrl}</link>
