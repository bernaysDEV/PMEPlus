...
router.post("/webhook", upload.any(), async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Respect the language setting configured for this trusted sender (ar, en)
    const language = (trustedSender.language || "ar") as "ar" | "en";
    ...
    console.log("[Email Agent] Analyzing and editing with Property ME editorial style...");
    ...
    if (editorialResult.qualityScore < qualityThreshold) {
      ...
      return res.status(200).json({
        success: false,
        message: "Content quality below threshold (Property ME standards)",
    ...
    const anonymousKeywords = ['صحيفة', 'صحيفه', 'بروبرتي ME', 'anonymous', 'مجهول'];
    ...
    // Newspaper account ID for anonymous publishing ("بروبرتي ME")
    ...
    if (publishAnonymously) {
      console.log("[Email Agent] 📰 Anonymous publishing detected (#صحيفة/#بروبرتي ME) - using newspaper account");
    ...
    // 📰 Reporter ID: Always default to newspaper account ("بروبرتي ME") for email-sourced articles
    const articleReporterId = NEWSPAPER_ACCOUNT_ID;
    console.log("[Email Agent] 📰 Reporter ID: بروبرتي ME (default for email articles)");
    ...
    const articleData: any = {
      ...
      reporterId: articleReporterId, // 📰 Default to بروبرتي ME for email articles
      ...
      if (trustedSender.autoPublish && article) {
        ...
        (async () => {
          ...
          sendEditorPublishAlert({
            ...
            language: editorialResult.language as 'ar' | 'en',
          });
    ...
    return res.status(200).json({
      ...
      message: trustedSender.autoPublish 
        ? "Article published successfully (edited with Property ME style)" 
        : "Article saved as draft (edited with Property ME style)",
      article: {
        id: article?.id,
