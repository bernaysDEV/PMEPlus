      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <p class="message">
              شكراً لتسجيلك في تطبيق بروبرتي ME.<br>
          ...
          <div class="footer">
            <p>© ${new Date().getFullYear()} بروبرتي ME - جميع الحقوق محفوظة</p>
          </div>
        </div>
      </body>
      </html>
...
شكراً لتسجيلك في تطبيق بروبرتي ME.
لتفعيل حسابك، يرجى إدخال رمز التفعيل التالي:
...
بروبرتي ME
    ...
    const result = await sendEmailNotification({
      to: email,
      subject: `رمز تفعيل حسابك في بروبرتي ME: ${code}`,
      html: htmlContent,
      text: textContent,
    });
...
// Helper: Generate 6-digit verification code (cryptographically secure)
...
router.post("/auth/register", async (req: Request, res: Response) => {
  try {
    ...
    // Validate password strength (centralised policy: min 12, top-100 reject)
    {
      const _pw = validatePasswordStrength(password);
      if (!_pw.ok) {
        return res.status(400).json({ success: false, message: _pw.message });
      }
    }
...
router.post("/auth/forgot-password", async (req: Request, res: Response) => {
  try {
    ...
    res.json({ 
      ...
      emailSent,
      // resetCode and userId intentionally omitted for security
    });
  } catch (error) {
...
router.post("/auth/reset-password", async (req: Request, res: Response) => {
  try {
    ...
    {
      const _pw = validatePasswordStrength(newPassword);
      if (!_pw.ok) {
        return res.status(400).json({ success: false, message: _pw.message });
      }
    }
