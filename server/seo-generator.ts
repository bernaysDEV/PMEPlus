...
// Supports Arabic (Claude) and English (GPT-4o)
...
function createUserPrompt(article: ArticleInput, language: "ar" | "en"): string {
  const prompts = {
...
export async function generateSeoMetadata(
  ...
  try {
    ...
    console.error(`[SEO] Primary model failed for ${language}: ${primaryMsg}`);
    ...
    try {
      ...
      console.error(`[SEO] Fallback model failed for ${language}: ${fallbackMsg}`);
...
export function validateSeoContent(content: SeoContent, language: "ar" | "en"): string[] {
  const errors: string[] = [];
