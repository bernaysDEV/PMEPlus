...
export interface IStorage {
  ...
  // SEO operations
  getArticleForSeo(id: string, language: "ar" | "en"): Promise<{
    id: string;
    title: string;
  ...
  // ============================================
  // Property ME Shorts (Reels) Operations - بروبرتي ME شورتس
  // ============================================
  ...
  // ==========================================
  // Calendar System - تقويم بروبرتي ME
  // ==========================================
...
/**
 * Pick the highest-precedence role name from the supplied list, used to
 * mirror onto the legacy `users.role` column. Order: superuser/admin roles
 * first, then editorial/staff roles, then reader. Unknown roles are
 * preferred over `'reader'` so custom roles are still reflected.
 */
const ROLE_PRECEDENCE: string[] = [
  'system_admin',
  'system.admin',
  'super_admin',
  'superadmin',
...
function pickPrimaryRoleName(names: string[]): string | undefined {
  ...
  // Unknown role — prefer the first non-reader entry, otherwise reader.
  return names.find((n) => n.toLowerCase() !== 'reader') ?? names[0];
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getUser(id: string): Promise<User | undefined> {
    try {
      ...
      // Resilience for fresh production DBs missing the legacy `role` column.
      // Replit's publish-time schema diff will add it; meanwhile keep auth working.
      if (err?.code === '42703' && /role/i.test(err?.message || '')) {
        const cols = getTableColumns(users) as Record<string, any>;
        const { role: _omit, ...safeCols } = cols;
        const [user] = await db.select(safeCols).from(users).where(eq(users.id, id));
        return user ? ({ ...user, role: 'reader' } as User) : undefined;
      }
      throw err;
    }
  }
  ...
  async getUserForAuth(identifier: string): Promise<User | undefined> {
    const where = or(eq(users.email, identifier.toLowerCase()), eq(users.id, identifier));
    try {
      ...
      if (err?.code === '42703' && /role/i.test(err?.message || '')) {
        const cols = getTableColumns(users) as Record<string, any>;
        const { role: _omit, ...safeCols } = cols;
        const [user] = await db.select(safeCols).from(users).where(where).limit(1);
        return user ? ({ ...user, role: 'reader' } as User) : undefined;
      }
      throw err;
    }
  ...
  async deleteUserPermanently(userId: string): Promise<{
    ...
    try {
      await db.transaction(async (tx) => {
        ...
        // 20. Delete dismissed continue reading
        ...
        // 21. Delete accessibility events
        ...
        // 22. Delete announcement dismissals
        ...
        // 25. Reassign articles to بروبرتي ME instead of anonymizing
        ...
        // 25b. Reassign English articles to بروبرتي ME
  ...
  async createUserWithRoles(userData: {
    ...
    // Resolve the primary role name to mirror onto the legacy `users.role`
    // column. Many older code paths (notifications, reporter dropdowns,
    // requireRole guards) still read this column, so keeping it in sync
    // with the RBAC assignment prevents staff accounts from silently
    // appearing as plain readers.
    const primaryRoleName = await this.resolvePrimaryRoleName(userData.roleIds);
  ...
  async updateUserRoles(userId: string, roleIds: string[], updatedBy: string, reason?: string): Promise<void> {
    await db.transaction(async (tx) => {
      ...
      // Mirror the primary role onto the legacy `users.role` column so older
      // code paths (notifications, reporter dropdowns, requireRole guards)
      // stay in sync with RBAC.
      const primaryName = pickPrimaryRoleName(newRoles.map((r) => r.name));
      if (primaryName) {
        await tx
          .update(users)
          .set({ role: primaryName, updatedAt: new Date() })
          .where(eq(users.id, userId));
      }
  ...
  /**
   * Look up the role names for the supplied role ids and return the
   * highest-precedence name to mirror onto the legacy `users.role` column.
   * Falls back to `'reader'` when no roles are supplied or none match.
   */
  private async resolvePrimaryRoleName(roleIds: string[]): Promise<string> {
    if (!roleIds || roleIds.length === 0) return 'reader';
    const rows = await db
      .select({ name: roles.name })
      .from(roles)
      .where(inArray(roles.id, roleIds));
    return pickPrimaryRoleName(rows.map((r) => r.name)) ?? 'reader';
  }
  ...
  async incrementArticleViews(id: string): Promise<void> {
    // One accepted click = exactly one view. No random inflation.
    await db
      .update(articles)
  ...
  // SEO operations
  async getArticleForSeo(id: string, language: "ar" | "en"): Promise<{
    id: string;
    title: string;
    subtitle?: string | null;
    content: string;
    ...
    const table = language === "ar" ? articles : enArticles;
  ...
  private calculateRank(totalPoints: number): string {
    if (totalPoints >= 2001) return "سفير بروبرتي ME";
  ...
  // ============================================
  // Calendar System Implementation - تقويم بروبرتي ME
  // ============================================
  ...
  async getEmailLanguageCounts(): Promise<{ ar: number; en: number }> {
    // Count articles created via email agent (articles that exist in emailWebhookLogs)
    // The database has separate tables for each language: articles (ar), enArticles (en)
  ...
  // ============================================
  // Real Estate Developers operations (نبض الشركات العقارية)
  // ============================================
  async getAllDevelopers(filter?: { isActive?: boolean; search?: string }): Promise<Developer[]> {
    const conditions: SQL[] = [];
    if (filter?.isActive !== undefined) {
      conditions.push(eq(developers.isActive, filter.isActive));
    }
    if (filter?.search && filter.search.trim().length > 0) {
      const term = `%${filter.search.trim()}%`;
      conditions.push(
        or(
          ilike(developers.nameAr, term),
          ilike(developers.nameEn, term),
          ilike(developers.slug, term),
        ) as SQL,
      );
    }
    const query = conditions.length > 0
      ? db.select().from(developers).where(and(...conditions))
      : db.select().from(developers);

    return await query.orderBy(asc(developers.sortOrder), asc(developers.nameAr));
  }

  async getDeveloperById(id: string): Promise<Developer | undefined> {
    const [row] = await db.select().from(developers).where(eq(developers.id, id)).limit(1);
    return row;
  }

  async getDeveloperBySlug(slug: string): Promise<Developer | undefined> {
    const [row] = await db.select().from(developers).where(eq(developers.slug, slug)).limit(1);
    return row;
  }
  ...
  async getArticleDevelopers(articleId: string): Promise<Developer[]> {
    const rows = await db
      .select({ d: developers })
      .from(articleDevelopers)
      .innerJoin(developers, eq(articleDevelopers.developerId, developers.id))
      .where(eq(articleDevelopers.articleId, articleId));
    return rows.map(r => r.d);
  }

  async setArticleDevelopers(articleId: string, developerIds: string[]): Promise<void> {
    // Replace the full set in a single transaction
    await db.transaction(async (tx) => {
      await tx.delete(articleDevelopers).where(eq(articleDevelopers.articleId, articleId));
      const unique = Array.from(new Set(developerIds.filter(Boolean)));
      if (unique.length === 0) return;
      await tx
        .insert(articleDevelopers)
        .values(unique.map(developerId => ({ articleId, developerId })))
        .onConflictDoNothing();
    });
  }
