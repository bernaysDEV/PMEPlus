...
async function migrateToR2() {
  ...
  // R2_BUCKET_NAME must be explicitly set so this script never silently
  // migrates into the legacy shared "sabq-media" bucket. Each brand owns
  // its own R2 bucket; there is no safe shared default.
  ...
  if (!r2Bucket) {
    console.error(
      'Missing R2_BUCKET_NAME. Provision a brand-owned R2 bucket and set ' +
      'R2_BUCKET_NAME before running this migration. The previous default ' +
      '("sabq-media") was removed to prevent uploads from co-mingling with ' +
      'the legacy sabq media bucket.'
    );
    process.exit(1);
  }
