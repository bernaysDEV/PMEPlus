...
import { INSTANCE_NAMESPACE } from "@shared/branding";
import { maskEmail } from "./logRedaction";
...
export function getSession() {
  ...
  if (redis) {
    // Namespace the session prefix so a Redis instance shared with other
    // brand deployments (e.g. the legacy "sabq" platform on the same Upstash
    // database) cannot accidentally accept or overwrite each other's
    // sessions. See `shared/branding.ts`.
    const sessionPrefix = `${INSTANCE_NAMESPACE}:sess:`;
    ...
    console.log(
      `[Session] Using Redis store (prefix: "${sessionPrefix}", fast, no DB pressure)`,
    );
  } else {
  ...
  passport.use(
    new LocalStrategy(
      ...
      async (email, password, done) => {
        try {
          console.log("🔍 LocalStrategy: Checking user:", maskEmail(email));
    ...
    passport.use(
      new GoogleStrategy(
        ...
        async (accessToken, refreshToken, profile, done) => {
          try {
            console.log("🔍 GoogleStrategy: Processing user:", maskEmail(profile.emails?.[0]?.value));
