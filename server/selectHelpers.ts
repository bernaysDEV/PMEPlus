 ...
 *
 * This rewrites `storage.googleapis.com` URLs through the CDN base configured
 * by the `PUBLIC_CDN_BASE_URL` env var (Cloudflare Polish + edge caching).
 * When that env var is unset, the original GCS URL is returned unchanged so
 * we never accidentally route public images through another brand's domain.
 */
