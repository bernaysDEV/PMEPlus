...
export const ROLE_DESCRIPTIONS_AR = {
  ...
  [ROLE_NAMES.CONTENT_MANAGER]: "إنشاء وتحرير أي مقال وإدارة التصنيفات والوسوم والوسائط وجدولة النشر",
  [ROLE_NAMES.PUBLISHER]: "إنشاء وتحرير وجدولة المقالات الخاصة فقط، دون نشر مقالات الآخرين",
  [ROLE_NAMES.MEDIA_MANAGER]: "إدارة المكتبة الإعلامية والألبومات",
  [ROLE_NAMES.READER]: "مستخدم عادي بدون صلاحيات تحريرية",
} as const;

// Roles that belong to the editorial / staff team (everything except `reader`).
// Used by admin UIs (StaffMembers, UsersManagement, AddUserDialog) to filter
// out the reader role from staff-management surfaces.
...
export function isStaffRole(roleName: string): boolean {
  return (STAFF_ROLE_NAMES as readonly string[]).includes(roleName);
}

// Subset of `STAFF_ROLE_NAMES` that admins are allowed to *manually* assign
// from in-app dialogs (Add Team Member, change-role). We exclude only
// `system_admin`, which is bootstrapped via `scripts/setup-system-admin.ts`
// and must never be granted from a routine UI click. The full-access
// `admin` ("مدير عام") role is intentionally included so authorised
// administrators can promote staff from the UI.
export const ASSIGNABLE_STAFF_ROLE_NAMES = STAFF_ROLE_NAMES.filter(
  (r) => r !== ROLE_NAMES.SYSTEM_ADMIN,
) as readonly StaffRoleName[];
...
export const PERMISSION_CODES = {
  ...
  SYSTEM_MANAGE_THEMES: "system.manage_themes",
  SYSTEM_VIEW_LOGS: "system.view_logs",

  // Users management aggregate (used by older user/staff endpoints)
...
export const ROLE_PERMISSIONS_MAP: Record<string, string[]> = {
  ...
  [ROLE_NAMES.ADMIN]: [
    ...
    PERMISSION_CODES.SYSTEM_MANAGE_ROLES,
    PERMISSION_CODES.SYSTEM_MANAGE_THEMES,
    PERMISSION_CODES.SYSTEM_VIEW_LOGS,
  ...
  // مدير محتوى — يدير المقالات والتصنيفات والوسوم والوسائط ويجدول النشر
  // (لا يصل إلى إدارة المستخدمين أو الإعدادات العامة)
  [ROLE_NAMES.CONTENT_MANAGER]: [
    PERMISSION_CODES.ARTICLES_VIEW,
    PERMISSION_CODES.ARTICLES_CREATE,
    PERMISSION_CODES.ARTICLES_EDIT_OWN,
    ...
    PERMISSION_CODES.TAGS_CREATE,
    PERMISSION_CODES.TAGS_UPDATE,
    PERMISSION_CODES.TAGS_DELETE,
    PERMISSION_CODES.MEDIA_VIEW,
    PERMISSION_CODES.MEDIA_UPLOAD,
    PERMISSION_CODES.MEDIA_EDIT,
    PERMISSION_CODES.OPINION_VIEW,
  ...
  // ناشر — ينشئ ويحرر ويجدل مقالاته الخاصة فقط (لا ينشر مقالات الآخرين)
  [ROLE_NAMES.PUBLISHER]: [
    PERMISSION_CODES.ARTICLES_VIEW,
    PERMISSION_CODES.ARTICLES_CREATE,
    PERMISSION_CODES.ARTICLES_EDIT_OWN,
    ...
    PERMISSION_CODES.MEDIA_VIEW,
    PERMISSION_CODES.MEDIA_UPLOAD,
    PERMISSION_CODES.COMMENTS_VIEW_OWN,
    PERMISSION_CODES.ANALYTICS_VIEW_OWN,
    PERMISSION_CODES.DASHBOARD_VIEW,
    PERMISSION_CODES.DASHBOARD_VIEW_STATS,
  ],
...
export const PERMISSION_LABELS_AR: Record<string, string> = {
  ...
  [PERMISSION_CODES.SYSTEM_MANAGE_ROLES]: "إدارة الأدوار",
  [PERMISSION_CODES.SYSTEM_MANAGE_THEMES]: "إدارة الثيمات",
  [PERMISSION_CODES.SYSTEM_VIEW_LOGS]: "عرض سجلات النظام",
  [PERMISSION_CODES.USERS_MANAGE]: "إدارة المستخدمين",
