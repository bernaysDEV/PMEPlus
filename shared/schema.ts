...
export const users = pgTable("users", {
  ...
  // Press Card fields (admin metadata for journalists)
...
export const staff = pgTable("staff", {
  ...
  // Bilingual fields (English and Arabic only)
  name: text("name").notNull(),
  nameAr: text("name_ar").notNull(),
  title: text("title"),
...
// ============================================
// Real Estate Developers (المطورون العقاريون)
// ============================================
// Used by the homepage "نبض الشركات العقارية" pulse block to track regional
// developers (إعمار، الدار، داماك، صبحا، نخيل، الديار القطرية، روشن…) and
// link them to articles for KPIs, leaderboard and ticker.
export const developers = pgTable("developers", {
  ...
  country: text("country").notNull(), // ISO 3166-1 alpha-2: AE, SA, QA, KW, BH, OM, EG
  countryNameAr: text("country_name_ar").notNull(), // الإمارات، السعودية…
  city: text("city"), // dubai, riyadh…
  cityNameAr: text("city_name_ar"),
  website: text("website"),
  brandColor: text("brand_color"), // hex like #0a3d62
  description: text("description"),
  ...
  index("idx_developers_slug").on(table.slug),
  index("idx_developers_country").on(table.country),
  index("idx_developers_active").on(table.isActive),
  index("idx_developers_featured").on(table.isFeatured),
  index("idx_developers_sort").on(table.sortOrder),
]);

// Junction table for article-developer many-to-many
export const articleDevelopers = pgTable("article_developers", {
  articleId: varchar("article_id").references(() => articles.id, { onDelete: "cascade" }).notNull(),
  developerId: varchar("developer_id").references(() => developers.id, { onDelete: "cascade" }).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (table) => ({
  pk: primaryKey({ columns: [table.articleId, table.developerId] }),
  articleIdx: index("idx_article_developers_article").on(table.articleId),
  developerIdx: index("idx_article_developers_developer").on(table.developerId),
}));
...
export const insertDeveloperSchema = createInsertSchema(developers).omit({
  ...
  slug: z.string().min(2, "المعرف مطلوب").regex(/^[a-z0-9-]+$/i, "المعرف يجب أن يحتوي على أحرف لاتينية وأرقام و - فقط"),
  country: z.string().length(2, "رمز الدولة يجب أن يكون من حرفين"),
  countryNameAr: z.string().min(2, "اسم الدولة بالعربية مطلوب"),
  brandColor: z.string().regex(/^#[0-9a-fA-F]{6}$/, "اللون يجب أن يكون بصيغة hex مثل #0a3d62").optional().or(z.literal("")).nullable(),
  website: z.string().url("الرابط غير صحيح").optional().or(z.literal("")).nullable(),
  logoUrl: z.string().url("رابط الشعار غير صحيح").optional().or(z.literal("")).nullable(),
  establishedYear: z
    .number()
    .int()
...
export const insertArticleDeveloperSchema = createInsertSchema(articleDevelopers).omit({
  createdAt: true,
});
...
export type ArticleDeveloper = typeof articleDevelopers.$inferSelect;
export type InsertArticleDeveloper = z.infer<typeof insertArticleDeveloperSchema>;
...
export const articlesRelations = relations(articles, ({ many }) => ({
  ...
  articleDevelopers: many(articleDevelopers),
}));
...
export const articleDevelopersRelations = relations(articleDevelopers, ({ one }) => ({
  article: one(articles, {
    fields: [articleDevelopers.articleId],
    references: [articles.id],
  }),
  developer: one(developers, {
    fields: [articleDevelopers.developerId],
...
export const paymentAlerts = pgTable("payment_alerts", {
  ...
  paymentType: varchar("payment_type", { length: 50 }).notNull(), // article
