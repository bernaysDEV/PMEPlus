...
// Editorial roles allowed to author/create articles. Used as a UI-level fallback
// for the "New Article" affordance so that a missing or out-of-sync
// `articles.create` permission does not silently hide the entry point for users
// whose role clearly implies authoring rights. Server routes remain protected by
// `requirePermission("articles.create")`.
export const ARTICLE_AUTHORING_ROLES = [
  "admin",
  "system_admin",
  "super_admin",
  "superadmin",
...
// True if the user can create articles by permission OR by an authoring role.
export function canCreateArticles(user: User | null | undefined): boolean {
  if (!user) return false;
  if (hasAnyPermission(user, "articles.create")) return true;
  return hasRole(user, ...ARTICLE_AUTHORING_ROLES);
}
