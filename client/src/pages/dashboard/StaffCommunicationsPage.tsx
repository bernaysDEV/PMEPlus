                ...
                deliveries.map((delivery) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        {delivery.status === "sent" && <CheckCircle className="h-4 w-4 text-muted-foreground" />}
                        {delivery.status === "failed" && (
                          <div className="flex items-center gap-1 text-destructive">
                            <XCircle className="h-4 w-4" />
                            {delivery.failureReason && (
                              <span className="text-xs">{delivery.failureReason}</span>
                            )}
                          </div>
                        )}
                        {delivery.status === "pending" && <AlertCircle className="h-4 w-4 text-muted-foreground" />}
                      </div>
                    </div>
                  );
                })
