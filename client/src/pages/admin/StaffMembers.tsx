...
// Canonical staff role list lives in `shared/rbac-constants.ts` so the API,
// the UI and the sync script all agree. We just alias it here for clarity.
const STAFF_ROLES = STAFF_ROLE_NAMES as readonly string[];
...
export default function StaffMembers() {
  ...
  const canChangeRoles = hasPermission(user, "users.change_role");
  ...
  // The role-change picker is restricted further: it never offers
  // `system_admin` / `admin`, which require a dedicated promotion script.
  const assignableStaffRoles = useMemo(() => {
    return roles.filter((r) =>
      (ASSIGNABLE_STAFF_ROLE_NAMES as readonly string[]).includes(r.name),
    );
  }, [roles]);
  ...
  const changeRoleMutation = useMutation({
    mutationFn: async ({
      ...
      return await apiRequest(`/api/admin/users/${userId}/roles`, {
        method: "PATCH",
        body: JSON.stringify({ roleIds, reason: reason || undefined }),
        headers: { "Content-Type": "application/json" },
      });
    },
    onSuccess: () => {
      ...
      toast({ title: "تم التحديث", description: "تم تغيير أدوار الموظف بنجاح" });
    ...
    onError: (error: any) => {
      toast({
        title: "خطأ",
        description: error?.message || "فشل تغيير الدور",
        variant: "destructive",
      });
    },
  });

  // When the "change roles" dialog opens we need to seed it with the user's
  // current set of role IDs so the admin can add/remove from the existing
  // assignment instead of overwriting it blindly.
  useEffect(() => {
    if (!roleDialogOpen || !selectedUser) return;
    let cancelled = false;
    (async () => {
      try {
        const res = await fetch(`/api/admin/users/${selectedUser.id}/roles`);
        if (!res.ok) {
          // Fall back to whatever single roleId we know about so the dialog
          // is not empty.
          if (!cancelled) {
            setSelectedRoleIds(selectedUser.roleId ? [selectedUser.roleId] : []);
          }
          return;
        }
        const data = await res.json();
        const ids = Array.isArray(data) ? data.map((r: Role) => r.id) : [];
        if (!cancelled) setSelectedRoleIds(ids);
      } catch {
        if (!cancelled) {
          setSelectedRoleIds(selectedUser.roleId ? [selectedUser.roleId] : []);
        }
    ...
    <DashboardLayout>
      <div className="p-6 space-y-6" dir="rtl">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold" data-testid="heading-staff-title">منسوبي بروبرتي ME</h1>
      ...
      <Dialog open={roleDialogOpen} onOpenChange={setRoleDialogOpen}>
        <DialogContent data-testid="dialog-staff-role">
          <DialogHeader>
            <DialogTitle>تغيير أدوار الموظف</DialogTitle>
            <DialogDescription>
              اختر الأدوار الجديدة للموظف "{selectedUser?.email}". يمكنك تحديد أكثر من دور.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>الأدوار *</Label>
              <div
                className="rounded-md border p-3 space-y-2 max-h-64 overflow-y-auto"
                data-testid="list-staff-roles"
              >
                ...
                {assignableStaffRoles.map((role) => {
                  ...
                  return (
                    <label
                      ...
                      data-testid={`checkbox-staff-role-${role.name}`}
                    >
                      <Checkbox
                        checked={checked}
            ...
            <div className="space-y-2">
              <Label htmlFor="role-change-reason">سبب التغيير (اختياري)</Label>
              <Input
                id="role-change-reason"
                value={roleChangeReason}
                onChange={(e) => setRoleChangeReason(e.target.value)}
                placeholder="مثال: تمت ترقية الموظف"
          ...
          <DialogFooter>
            <Button
              ...
              data-testid="button-staff-role-cancel"
            >
              إلغاء
            </Button>
            <Button
              ...
              disabled={
                selectedRoleIds.length === 0 || changeRoleMutation.isPending
              }
      ...
      <AddUserDialog
        ...
        availableRoleNames={ASSIGNABLE_STAFF_ROLE_NAMES}
        title="إضافة عضو جديد"
        description="أنشئ حساب موظف جديد واختر دوره (أو أدواره) ضمن الفريق."
      />
