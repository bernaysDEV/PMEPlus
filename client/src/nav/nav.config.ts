  ...
  // ===== المحتوى / Content =====
  {
    ...
    children: [
      ...
      },
      {
        ...
        path: "/dashboard/articles/new",
        icon: PlusCircle,
        // Editorial roles always see "New Article" — this matches the dashboard
        // page header and Quick Actions card. The nav filter uses OR semantics
        // between roles and permissions, so a custom role with `articles.create`
        // (but not in this list) still sees the entry, AND an editorial role
        // missing the permission also sees it as a fallback. Server routes
        // remain protected by `requirePermission("articles.create")`.
      ...
      },
      {
        ...
        labelKey: "nav.real_estate_developers",
        labelAr: "المطورون العقاريون",
        path: "/dashboard/real-estate-developers",
        icon: Building2,
        roles: ["admin"],
      },
      {
