...
function itemPassesAccessCheck(
  ...
  const hasPermissionMatch =
    ...
    !!userPermissions?.some((p) => item.permissions!.includes(p));

  const hasRoleMatch = item.roles.includes(role);

  // OR semantics: an item is accessible if EITHER the user has a matching
  // permission OR their role is explicitly listed. The role list in nav.config
  // is the curated audience for the item; using OR keeps the entry visible
  // for those roles even when the underlying permission seed/RBAC sync drifts
  // (e.g. an admin without `articles.create` still sees "New Article").
  // Items that should remain strictly permission-gated simply omit `roles`.
  if (item.permissions && item.permissions.length > 0) {
    return hasPermissionMatch || hasRoleMatch;
  }

  // No permissions defined - use role-based check only
  return hasRoleMatch;
}
