import { Menu, User, LogOut, LayoutDashboard, Bell, Newspaper, Users, MessageSquare, Brain, Sparkles, ExternalLink, Zap, Home, Clock, BookOpen, Bookmark, ChevronLeft, FolderOpen, Search } from "lucide-react";
...
export function Header({ user, onMenuClick }: HeaderProps) {
  ...
  // Soft "go home" handler — when the user is already on `/` we stay on the
  // page and just scroll back to the top + invalidate the homepage query so
  // they get fresh content. We deliberately avoid `window.location.reload()`
  // so the user doesn't lose state, scroll position is animated rather than
  // jarring, and below-the-fold lazy chunks aren't re-downloaded on every
  // logo tap.
  ...
  return (
    <header role="banner" aria-label="رأس الصفحة الرئيسي" className="sticky top-0 z-50 w-full border-b border-sidebar-border bg-sidebar text-sidebar-foreground" dir="rtl">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden md:flex items-center gap-2">
              ...
              {user ? (
                ...
                <Button asChild variant="outline" className="bg-transparent border-sidebar-border text-sidebar-foreground" data-testid="button-login">
                  <a href="/login">
                    تسجيل الدخول
                  </a>
                </Button>
